...
function TabletBlog(targetTablet) {
  ...
  useEffect(() => {
    // Set processors and graphics as-is (usually array of arrays)
    if (targetTablet.processor?.length)
      setFinalProcessor(targetTablet.processor[0]);
    else setFinalProcessor([]);

    if (targetTablet.graphic?.length) setFinalGraphic(targetTablet.graphic[0]);
    else setFinalGraphic([]);

    // Combine ram, storage, price arrays into sortable flat array
    const ramArr = targetTablet.ram?.[0] || [];
    const storageArr = targetTablet.storage?.[0] || [];
    const priceArr = targetTablet.price?.[0] || [];
      ...
      <div className="flex w-full max-w-screen-xl">
        <div className="flex-1 p-4 bg-white rounded-lg shadow-md">
          ...
          <p className="text-gray-600 text-sm md:text-sm my-2">
            Explore the{" "}
            {targetTablet.item_categorie === "flagship"
              ? "outstanding "
              : "great "}
          ...
          <div className="py-4 border-t-2 w-full">
            <div className="flex items-center justify-between mb-2">
              <h2 className="underline-animations text-xl font-bold w-auto inline-block mb-3">
                {targetTablet.name + " "}Specifications
              </h2>
            </div>
            <div className="flex flex-col space-y-2">
              {[
                { label: "Dimension", value: targetTablet.dimension },
                { label: "Build", value: targetTablet.build },
                { label: "Weight", value: targetTablet.weight },
                { label: "Type", value: targetTablet.dtype },
                { label: "Size", value: targetTablet.size },
                { label: "Resolution", value: targetTablet.resolution },
                { label: "Front Camera", value: targetTablet.frontcamera },
                { label: "Main Camera", value: targetTablet.maincamera },
                { label: "Video", value: targetTablet.video },
                { label: "OS", value: targetTablet.os },
                {
                  label: "Processor",
                ...
                { label: "Capacity", value: targetTablet.capacity },
                { label: "Charging", value: targetTablet.charging },
                { label: "Wi-Fi", value: targetTablet.wifi },
                { label: "Bluetooth", value: targetTablet.bluetooth },
                { label: "Type-C", value: targetTablet.typec },
                { label: "Audio Jack", value: targetTablet.audiojack },
              ].map((spec, index) => (
                <InfoSection
                  key={index}
                  label={spec.label}
          ...
          <div className="py-4">
            {targetTablet?.descriptions?.map((item, index) => (
